...
public class FmtHandler
{
    ...
    public static string? GetShareUri(ProfileItem item)
    {
        try
        {
            var url = item.ConfigType switch
            {
                ...
                EConfigType.OpenVPN => OpenVPNFmt.ToUri(item),
                _ => null,
            };
    ...
    public static ProfileItem? ResolveConfig(string config, out string msg)
    {
        ...
        try
        {
            ...
            else if (str.StartsWith(Global.ProtocolShares[EConfigType.OpenVPN])
                     || str.StartsWith(Global.OpenVPNProtocolShare))
            {
                return OpenVPNFmt.Resolve(str, out msg);
            }
            else
            {
