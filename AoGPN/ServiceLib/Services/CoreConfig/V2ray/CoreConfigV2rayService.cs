...
public partial class CoreConfigV2rayService(CoreConfigContext context)
{
    ...
    public RetResult GenerateClientConfigContent()
    {
        ...
        try
        {
            ...
            // Always append the final rule (0-65535 → proxy, or the balancer tag):
            // a missing catch-all lets unmatched traffic fall through, which in
            // Global VPN mode is exactly the silent-leak symptom users hit when
            // their active routing profile carries a stale "0-65535 direct" rule
            // (or no catch-all at all). First-match-wins still honours any
            // user-defined catch-all that precedes it.
