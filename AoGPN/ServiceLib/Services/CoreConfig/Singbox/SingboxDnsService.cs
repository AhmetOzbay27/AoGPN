...
public partial class CoreConfigSingboxService
{
    private void GenDns()
    {
        try
        {
            ...
            // Force IPv4-only DNS strategy when IPv6 is disabled — prevents
            // "missing IPv6 local address" errors that kill all DNS resolution.
            if (_config.TunModeItem.EnableIPv6Address != true)
            {
                _coreConfig.dns.strategy = "ipv4_only";
                DiagLog.Write("DNS strategy forced: ipv4_only (IPv6 disabled by user)");
            }
    ...
    private void GenDnsRules()
    {
        ...
        // When the user has NOT enabled IPv6 through the proxy, reject AAAA
        // (IPv6) DNS queries from proxy-routed processes. This prevents DNS
        // from returning AAAA records that Chrome/Edge would use to connect
        // via native IPv6 — a complete TUN bypass. Combined with the TUN-level
        // ::/0 block in routing rules, this provides defence in depth.
        if (context.IsTunEnabled && _config.TunModeItem.EnableIPv6Address != true)
        {
            var dnsRouting = context.RoutingItem;
            if (dnsRouting != null)
            {
                var dnsRuleItems = JsonUtils.Deserialize<List<RulesItem>>(dnsRouting.RuleSet) ?? [];
                var proxyProcessNames = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
                foreach (var ruleItem in dnsRuleItems)
                {
                    if (!ruleItem.Enabled || ruleItem.RuleType != ERuleType.Routing)
                        continue;
                    var tag = ruleItem.OutboundTag ?? Global.ProxyTag;
                    if (tag == Global.DirectTag || tag == Global.BlockTag)
                        continue;
                    foreach (var p in ruleItem.Process ?? [])
                    {
                        var norm = Utils.GetExeName(p);
                        if (!norm.IsNullOrEmpty())
                            proxyProcessNames.Add(norm);
                    }
                }
                if (proxyProcessNames.Count > 0)
                {
                    _coreConfig.dns.rules.Add(new Rule4Sbox
                    {
                        process_name = proxyProcessNames.ToList(),
                        query_type = [28], // AAAA
                        action = "reject",
                    });
                    DiagLog.Write($"DNS aaaaBlock: rejecting AAAA for {proxyProcessNames.Count} process(es): [{string.Join(", ", proxyProcessNames)}]");
                }
            }
        }
