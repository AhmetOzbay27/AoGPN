...
public sealed class AppManager
{
    ...
    /// <summary>
    /// The single application-owned core lifecycle facade. It is assigned by the
    /// main view model after configuration load and reused by shutdown/update flows.
    /// </summary>
...
#pragma warning restore CS0618
    ...
    {
        Logging.SaveLog($"AoGPN start up | {Utils.GetRuntimeInfo()}");
        Logging.LoggingEnabled(_config.GuiItem.EnableLog);
        Logging.VerboseLoggingEnabled(_config.GuiItem.EnableVerboseLog);
    ...
    /// <summary>
    /// Flushes every queued write (e.g. from the connection list) plus this final
    /// save, and lets window state subscribers persist before shutdown.
    /// </summary>
    ...
    public async Task AppExitAsync(bool needShutdown)
    {
        try
        {
            ...
            // Each cleanup step is isolated so a single failure can never skip the
            // remaining shutdown work: the OS proxy must be cleared, config/DB
            // flushed and the core stopped even if an earlier step throws.
            await RunExitStepAsync("clear system proxy", () => SysProxyHandler.UpdateSysProxy(_config, true));
            await RunExitStepAsync("flush state", FlushStateAsync);
            await RunExitStepAsync("stop core", () => StopCoreAsync());
            await RunExitStepAsync("close statistics", () =>
            {
        ...
        {
            // Unexpected failure outside the isolated steps (e.g. logging itself).
            Logging.SaveLog("AppExitAsync unexpected failure", ex);
        }
        finally
        {
            if (needShutdown)
    ...
    /// <summary>
    /// Per-step timeout for <see cref="AppExitAsync"/>. A stuck lifecycle step
    /// (proxy gate, config-flush signal, core-stop gate) previously blocked the
    /// whole shutdown for minutes. Each step is now bounded so the remaining
    /// steps always run. Test seam: lowered in unit tests.
    /// </summary>
    internal static TimeSpan ExitStepTimeout { get; set; } = TimeSpan.FromSeconds(20);

    /// <summary>
    /// Runs one best-effort shutdown step, logging (never rethrowing) failures so
    /// the remaining exit steps always execute. Each step logs start/done and
    /// times out after <see cref="ExitStepTimeout"/> — the stuck step is named in
    /// the log so the culprit is identifiable.
    /// </summary>
    private static async Task RunExitStepAsync(string stepName, Func<Task> step)
    {
        var sw = Stopwatch.StartNew();
        Logging.SaveLog($"AppExitAsync step '{stepName}' start");
        try
        {
            await step().WaitAsync(ExitStepTimeout);
            Logging.SaveLog($"AppExitAsync step '{stepName}' done in {sw.ElapsedMilliseconds} ms");
        }
        catch (TimeoutException)
        {
            Logging.SaveLog($"AppExitAsync step '{stepName}' TIMED OUT after {ExitStepTimeout.TotalSeconds}s — "
                + "shutdown continues without it.");
        }
        catch (Exception ex)
        {
            Logging.SaveLog($"AppExitAsync step '{stepName}' failed", ex);
        }
    }
    ...
    /// <summary>
    /// Stops the application-owned core host. The direct CoreManager fallback is
    /// retained for update/diagnostic flows that run before normal initialization.
    /// </summary>
    public async Task StopCoreAsync(CancellationToken cancellationToken = default)
    {
        if (CoreEngineHost is not null)
        {
            await CoreEngineHost.StopAsync(cancellationToken);
            return;
        }
