...
public class CoreConfigSingboxServiceTests
{
    ...
    [Fact]
    public void GenerateClientConfigContent_TunInboundHasNoLegacySniffFieldsAndRouteCarriesSniffAction()
    {
        var config = CoreConfigTestFactory.CreateConfig(ECoreType.sing_box);
        CoreConfigTestFactory.BindAppManagerConfig(config);
        var node = CoreConfigTestFactory.CreateSocksNode(ECoreType.sing_box);
        ...
        var tun = cfg.inbounds.FirstOrDefault(i => i.type == "tun");
        tun.Should().NotBeNull("TUN mode must emit a tun inbound");

        // sing-box 1.11 deprecated the inbound sniff fields and 1.13 removed them
        // (the core exits with FATAL when they are present). The model has no such
        // properties anymore, so this asserts on the emitted JSON to also catch a
        // regression in the embedded tun_singbox_inbound sample.
        var tunJson = JsonUtils.Serialize(tun, false);
        tunJson.Should().NotContain("sniff",
            "TUN inbound must not carry legacy sniff/sniff_override_destination fields");

        // Sniffing is now a route action and must be present for the GPN path.
        cfg.route.rules.Should().Contain(r => r.action == "sniff",
            "sniffing must be emitted as a route action since sing-box 1.13");
    }
    ...
    [Fact]
    public void GenerateClientConfigContent_VlessReality_NoFingerprint_FallsBackToChromeUtls()
    {
        // A VLESS+REALITY node with no fingerprint (neither node nor configured
        // default) must still present a browser uTLS fingerprint. Without this the
        // handshake is a raw Go/uTLS hello that Cloudflare flags as "Suspicious
        // behavior" - the exact failure v2rayN avoids because it always masks with a
        // browser hello.
        var config = CoreConfigTestFactory.CreateConfig(ECoreType.sing_box);
        config.CoreBasicItem.DefFingerprint = string.Empty;
        CoreConfigTestFactory.BindAppManagerConfig(config);

        var node = new ProfileItem
        {
            IndexId = "n-reality-nofp",
            ConfigType = EConfigType.VLESS,
            CoreType = ECoreType.sing_box,
            Remarks = "reality-nofp",
        ...
        result.Success.Should().BeTrue($"ret msg: {result.Msg}");
        var cfg = JsonUtils.Deserialize<SingboxConfig>(result.Data!.ToString())!;
        var proxy = cfg.outbounds.First(o => o.tag == Global.ProxyTag && o.type == "vless");
        proxy.tls.Should().NotBeNull();
        proxy.tls!.utls.Should().NotBeNull("Reality must mask with a browser uTLS fingerprint");
        proxy.tls!.utls!.enabled.Should().BeTrue();
        proxy.tls.utls!.fingerprint.Should().Be("chrome");
        proxy.tls.reality.Should().NotBeNull();
        proxy.tls.reality!.enabled.Should().BeTrue();
    }
