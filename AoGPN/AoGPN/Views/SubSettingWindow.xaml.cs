...
public partial class SubSettingWindow
{
    public SubSettingWindow()
    {
        ...
        this.WhenActivated(disposables =>
        {
            ...
            this.BindCommand(ViewModel, vm => vm.ImportPublicSourcesCmd, v => v.menuImportPublicSources).DisposeWith(disposables);

            ViewModel.ImportPublicSourcesInteraction.RegisterHandler(async interaction =>
            {
                var sourceList = interaction.Input;

                // Build display text for the dialog
                var sb = new System.Text.StringBuilder();
                foreach (var s in sourceList)
                {
                    var flag = s.Flag.IsNotEmpty() ? s.Flag + " " : "";
                    sb.AppendLine($"{flag}{s.Name}  [{s.Category}]");
                }

                var result = UI.ShowYesNo(
                    $"{ResUI.menuImportPublicSources}\n\n{sb}") != MessageBoxResult.No;

                interaction.SetOutput(result ? sourceList : []);
