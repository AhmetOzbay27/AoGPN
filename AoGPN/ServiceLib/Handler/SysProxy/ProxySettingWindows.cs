...
public static class ProxySettingWindows
{
    ...
    /// <summary>
    /// Writes the proxy configuration to the registry and immediately broadcasts
    /// the change to all running applications. This ensures the proxy takes effect
    /// in sub-second time instead of waiting for the next WinINET polling cycle
    ...
    private static bool SetProxyFallback(string? strProxy, string? exceptions, int type)
    {
        ...
        // Broadcast the registry change immediately so applications pick it up
        // without waiting for the next poll cycle (the old code wrote the registry
        // keys silently, which caused a multi-second delay before Chrome/Edge/etc.
        // noticed the proxy had changed).
        try
        {
        ...
        {
            // Broadcast is best-effort — the registry keys are already committed.
        }
    ...
    public static bool SetProxy(string? strProxy, string? exceptions, int type)
    {
        try
        {
            ...
            if (!result)
            {
                // InternetSetOption reported failure; fall back to registry-only
                // write + immediate broadcast so the change still takes effect
                // without the multi-second poll-cycle delay.
        ...
        {
            Logging.SaveLog("ProxySettingWindows.SetProxy failed", ex);
            _ = SetProxyFallback(strProxy, exceptions, type);
            return false;
        }
    }
