...
stage_runtime_assets() {
  ...
  if [[ "$FORCE_NETCORE" -eq 0 ]]; then
    if populate_assets_zip_mode "$outroot" "$rid"; then
      echo "[*] Using AoGPN bundle bin assets."
    else
...
write_launcher_file() {
  ...
  install -m 755 /dev/stdin "$stage/usr/bin/aogpn" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
DIR="/opt/AoGPN"
cd "$DIR"
...
write_desktop_file() {
  ...
  install -m 644 /dev/stdin "$stage/usr/share/applications/aogpn.desktop" <<'EOF'
[Desktop Entry]
Type=Application
Name=AoGPN
Comment=AoGPN for Debian GNU Linux
Exec=aogpn
Icon=aogpn
Terminal=false
Categories=Network;
...
package_binary() {
  ...
  mkdir -p "$stage/opt/AoGPN" "$stage/usr/bin" "$stage/usr/share/applications" "$stage/usr/share/icons/hicolor/256x256/apps" "$debian_dir"
  cp -a "$pubdir/." "$stage/opt/AoGPN/"

  project_dir="$(cd "$(dirname "$PROJECT")" && pwd)"
  icon_candidate="$project_dir/AoGPN.png"
  [[ -f "$icon_candidate" ]] && cp "$icon_candidate" "$stage/usr/share/icons/hicolor/256x256/apps/aogpn.png" || true

  stage_runtime_assets "$stage/opt/AoGPN" "$rid"
...
Section: net
Priority: optional
Maintainer: Ahmet Ozbay <AhmetOzbay27@users.noreply.github.com>
...
EOF
  ...
  mapfile -t ELF_FILES < <(
    find "$stage/opt/AoGPN" -type f \( -name "*.so*" -o -perm -111 \) ! -name 'libcoreclrtraceptprovider.so'
  )
...
Architecture: ${deb_arch}
Maintainer: Ahmet Ozbay <AhmetOzbay27@users.noreply.github.com>
Homepage: https://github.com/AhmetOzbay27/AoGPN
Section: net
Priority: optional
Depends: ${final_depends}
Description: AoGPN (Avalonia) GUI client for Linux
 Support vless / vmess / Trojan / http / socks / Anytls / Hysteria2 /
 Shadowsocks / tuic / WireGuard.
EOF

  find "$stage/opt/AoGPN" -type d -exec chmod 0755 {} +
  find "$stage/opt/AoGPN" -type f -exec chmod 0644 {} +
  [[ -f "$stage/opt/AoGPN/AoGPN" ]] && chmod 0755 "$stage/opt/AoGPN/AoGPN" || true

  deb_out="$OUTPUT_DIR/aogpn_${VERSION}_${deb_arch}.deb"
