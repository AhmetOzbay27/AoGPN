...
public class WireguardFmtTests
{
    ...
    public void ResolveConfig_ShouldParsePeersAndIgnoreInlineComments()
    {
        ...
        first.GetProtocolExtra().WgPersistentKeepalive.Should().Be(25);

        var second = resolved[1];
        second.Address.Should().Be("example.com");
        second.Port.Should().Be(12345);
        // İkinci peer'da PersistentKeepalive yok → null kalır (varsayılan kapalı).
        second.GetProtocolExtra().WgPersistentKeepalive.Should().BeNull();
    }
}
