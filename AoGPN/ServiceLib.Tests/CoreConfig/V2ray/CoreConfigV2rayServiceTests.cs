...
public class CoreConfigV2rayServiceTests
{
    ...
    public void GenerateClientConfigContent_HttpOutbound_ShouldEmitHeadersInSettings()
    {
        ...
        {
            HttpHeaders = "{\"User-Agent\":\"AoGPN\",\"Set-Cookie\":[\"a=1\",\"b=2\"]}",
        });
        var context = CoreConfigTestFactory.CreateContext(config, node, ECoreType.Xray);
        ...
        headers["User-Agent"]!.GetValue<string>().Should().Be("AoGPN");
    ...
    [Fact]
    public void GenerateClientConfigContent_CatchAllDirectAfterProcessRule_ShouldEmitProcessRuleFirst()
    {
        // Regression: older/wrong writes can persist the GPN catch-all
        // (port 0-65535 -> direct) at the START of the RuleSet. In first-match-wins
        // routing that shadows the process -> proxy rule, so listed apps exit
        // direct instead of through the proxy. The generator must defer the
        // catch-all to the end so specific process/domain/IP rules are evaluated
        // first.
        var config = CoreConfigTestFactory.CreateConfig(ECoreType.Xray);
        CoreConfigTestFactory.BindAppManagerConfig(config);
        var node = CoreConfigTestFactory.CreateVmessNode(ECoreType.Xray, "n-main", "main");
        ...
        var context = CoreConfigTestFactory.CreateContext(config, node, ECoreType.Xray) with
        {
            RoutingItem = new RoutingItem
            {
                Id = "r-gpn-catchall-first",
                Remarks = "gpn-catchall-first",
                RuleSet = JsonUtils.Serialize(new List<RulesItem>
                {
                    // Catch-all DIRECT at index 0 (the bug reproduced by the GPN DB).
                    new()
                    {
                        Enabled = true,
                        RuleType = ERuleType.Routing,
                    ...
                    // Listed app routed through the proxy.
                    new()
                    {
                        Enabled = true,
                        RuleType = ERuleType.Routing,
                        OutboundTag = Global.ProxyTag,
                        Process = ["Freebuff.exe"],
                        Network = "tcp,udp",
                    }
                }),
                DomainStrategy = Global.AsIs,
        ...
        var processIndex = cfg.routing.rules
            .FindIndex(r => r.process != null && r.process.Contains("Freebuff.exe"));
        var catchAllIndex = cfg.routing.rules
            .FindIndex(r => r.port == "0-65535" && r.outboundTag == Global.DirectTag);

        processIndex.Should().BeGreaterThanOrEqualTo(0, "the process -> proxy rule must be emitted");
        catchAllIndex.Should().BeGreaterThanOrEqualTo(0, "the direct catch-all must be emitted");
        // The catch-all must come after the process rule so it does not shadow it.
        processIndex.Should().BeLessThan(catchAllIndex);
    }
