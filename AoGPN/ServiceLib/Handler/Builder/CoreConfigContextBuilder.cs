...
public class CoreConfigContextBuilder
{
    ...
    {
        return await Build(config, node, proxyOnly: false);
    }

    /// <summary>
    ///     Builds a <see cref="CoreConfigContext" /> for the given node, optionally in
    ///     "proxy-only" mode.
    ///     <para>
    ///     A proxy-only core runs the selected node as a plain SOCKS/HTTP outbound (the
    ///     system-proxy path) with TUN explicitly off. In that mode the GPN/TUN
    ///     sing-box requirement does not apply, so the node keeps whatever core type it
    ///     was actually configured for (normally Xray for VLESS/Reality — exactly what
    ///     v2rayN runs). Otherwise <c>Build(config, node)</c> behaves identically to the
    ///     existing <see cref="Build(Config!, ProfileItem!)" /> path.
    ///     </para>
    /// </summary>
    public static async Task<CoreConfigContextBuilderResult> Build(Config config, ProfileItem node, bool proxyOnly)
    {
        var runCoreType = AppManager.Instance.GetCoreType(node, node.ConfigType);

        // GPN/TUN capture is mihomo only. The GPN process_name routing rules and
        // the per-process split tunnel are generated by the mihomo YAML builder,
        // and Xray has no native TUN support on Windows. mihomo supports every
        // in-app protocol the app actually exposes (VLESS, VMess, Trojan,
        // Shadowsocks, Hysteria2, TUIC, WireGuard, SOCKS), so when the TUN is
        // enabled the main core must be mihomo — never Xray/v2fly. Custom configs
        // and OpenVPN keep their own core.
        // In proxy-only mode TUN is never used, so this forcing does not apply
        // and the node keeps its natural core type.
        if (!proxyOnly
            && config.TunModeItem.EnableTun
            && node.ConfigType != EConfigType.Custom
            && runCoreType is ECoreType.Xray or ECoreType.v2fly or ECoreType.v2fly_v5)
        {
            runCoreType = ECoreType.mihomo;
        }

        var isTunEnabled = !proxyOnly && config.TunModeItem.EnableTun;
        var coreType = runCoreType == ECoreType.mihomo ? ECoreType.mihomo : ECoreType.Xray;
        var context = new CoreConfigContext
        {
            ...
            ProtectCoreTypeList = isTunEnabled ? [ECoreType.Xray, ECoreType.mihomo] : []
        };
        ...
        // OpenVPN has its own native configuration and routing stack. Its profile
        // must not be interpreted as an Xray/mihomo outbound or inherit their
        // routing-rule/pre-SOCKS graph.
    ...
    {
        // OpenVPN provides the tunnel itself; attaching the legacy protection
        // helper would create a second competing TUN lifecycle.
        if (nodeContext.RunCoreType == ECoreType.openvpn)
        {
            return null;
        }

        // mihomo owns its TUN (auto-route + interface-name) and dials out with
        // interface-name pinning — a legacy protect helper would fight
        // mihomo for the Wintun adapter and the default route.
        if (nodeContext.RunCoreType == ECoreType.mihomo)
        {
            return null;
        }
        ...
        // Use the ALREADY-RESOLVED core type from the main context: when TUN is
        // enabled the main core is forced to mihomo, and GetPreSocksItem only
        // builds a helper for non-mihomo cores. Recomputing via GetCoreType
        // here would see the profile's raw Xray type and wrongly spawn a
        // pre-socks helper on top of the mihomo main core.
        var coreType = nodeContext.RunCoreType;
