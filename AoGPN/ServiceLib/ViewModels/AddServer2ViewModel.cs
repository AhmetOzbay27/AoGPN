...
public class AddServer2ViewModel : MyReactiveObject, ICloseable
{
    ...
    public async Task BrowseServer(string fileName)
    {
        ...
        if (string.Equals(Path.GetExtension(fileName), ".ovpn", StringComparison.OrdinalIgnoreCase))
        {
            // OpenVPN .ovpn files are already complete native-core configs;
            // preserve them as Custom while selecting the OpenVPN launcher.
            item.CoreType = ECoreType.openvpn;
            CoreType = nameof(ECoreType.openvpn);
        }
