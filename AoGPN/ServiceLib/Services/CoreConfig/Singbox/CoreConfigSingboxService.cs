...
public partial class CoreConfigSingboxService(CoreConfigContext context)
{
    ...
    public RetResult GenerateClientConfigContent()
    {
        ...
        try
        {
            ...
            DiagLog.Write($"CFG_GEN start: tunEnabled={context.IsTunEnabled} coreType={context.RunCoreType} node={_node.Remarks}");
            ...
            var finalJson = ApplyFullConfigTemplate();
            ret.Data = finalJson;

            // Dump the full config for diagnostics — critical for debugging
            // split-tunnel leaks.
            try
            {
                var cfgDumpPath = System.IO.Path.Combine(
                    System.IO.Path.GetDirectoryName(DiagLog.FilePath) ?? ".",
                    "ao_diag_config.json");
                System.IO.File.WriteAllText(cfgDumpPath,
                    ((string)finalJson)
                        .Replace("\"password\":\"", "\"password\":\"***")
                        .Replace("\"uuid\":\"", "\"uuid\":\"***")
                        .Replace("\"private_key\":\"", "\"private_key\":\"***"));
                DiagLog.Write($"CFG_DUMP wrote config to {cfgDumpPath}");
            }
            catch { /* best effort */ }
            return ret;
        }
