...
public static class ConfigHandler
{
    ...
    /// <summary>
    /// Set when the config was recovered from the backup or reset to defaults during
    /// load; the main window shows it once after startup.
    /// </summary>
    public static string? LoadWarning { get; set; }
    ...
    public static Config? LoadConfig()
    {
        ...
        var backupPath = $"{resPath}.bak";

        Config? config = null;
        var fileExists = File.Exists(resPath);
        var result = EmbedUtils.LoadResource(resPath);
        if (result.IsNotEmpty())
        {
            config = JsonUtils.Deserialize<Config>(result);
        }

        if (config is null && fileExists)
        {
            // Corrupt or empty main file — try the last backup written by SaveConfig.
            var backup = EmbedUtils.LoadResource(backupPath);
            if (backup.IsNotEmpty())
            {
                config = JsonUtils.Deserialize<Config>(backup);
            }

            if (config is not null)
            {
                LoadWarning = ResUI.ConfigRecoveredFromBackup;
                Logging.SaveLog("LoadConfig recovered from backup");
                TryRepairMainConfig(resPath, config);
            }
            else
            {
                LoadWarning = ResUI.ConfigResetToDefaults;
                Logging.SaveLog("LoadConfig corrupted, resetting to defaults");
            }
        }
        ...
        {
            EnableTun = false,
            Mtu = WindowsTunStabilityPolicy.DefaultMtu,
            Stack = WindowsTunStabilityPolicy.DefaultStack,
        ...
        config.GpnCaptureItem ??= new();
        config.MsgUIItem ??= new();

        config.UiItem ??= new();
        // The tray flags (Minimize2Tray / Hide2TrayWhenClose) default to false on
        // a fresh UiItem: closing X or minimizing stays in the taskbar / fully
        // exits unless the user opts in. Never force them on during load —
        // close-to-tray in particular must remain user-elected.
        config.UiItem.MainColumnItem ??= [];
        config.UiItem.WindowSizeItem ??= [];

        // The modern gaming/network workspace is dark by default. Existing user choices remain untouched.
        ...
        config.ConnectionItem.ProtocolPreference = ConnectionProtocolPreference.Normalize(config.ConnectionItem.ProtocolPreference);
        config.ConnectionItem.AutoReconnectMaxAttempts = Math.Clamp(config.ConnectionItem.AutoReconnectMaxAttempts, 1, 10);
        if (Utils.IsWindows())
        {
    ...
    /// <summary>Best-effort repair of the main config file after recovering from the backup.</summary>
    private static void TryRepairMainConfig(string resPath, Config config)
    {
        try
        {
    ...
    public static async Task<int> SaveConfig(Config config)
    {
        try
        {
            ...
            // Keep a copy of the previous good file so a corrupt config can be
            // recovered on the next launch.
            if (File.Exists(resPath))
            {
                try
                {
                    File.Copy(resPath, backupPath, true);
                }
                catch (Exception ex)
                {
                    Logging.SaveLog(_tag, ex);
            ...
            // ConfigureAwait(false): this write is pure I/O that never needs the
            // caller's SynchronizationContext. Without it, a save requested from
            // a thread that then blocks synchronously on the flush (the exit
            // path) deadlocks — the continuation is posted to the blocked
            // dispatcher and the write never completes (observed live 2026-09-03).
    ...
    public static async Task<int> AddServer(Config config, ProfileItem profileItem)
    {
        ...
        var ret = item.ConfigType switch
        {
            ...
            EConfigType.OpenVPN => await AddOpenVPNServer(config, item),
            _ => -1,
        };
        return ret;
    }
    ...
    public static async Task<int> AddWireguardServer(Config config, ProfileItem profileItem, bool toFile = true)
    {
        ...
        var wgKeepalive = profileItem.GetProtocolExtra().WgPersistentKeepalive;
        profileItem.SetProtocolExtra(profileItem.GetProtocolExtra() with
        {
            ...
            WgPersistentKeepalive = wgKeepalive is > 0 and <= 65535 ? wgKeepalive : null,
        });
    ...
    /// <summary>
    /// Add or edit an OpenVPN server profile.
    /// OpenVPN is launched by its own native core and is intentionally not routed
    /// through Xray or sing-box; this keeps its TUN lifecycle independent.
    /// </summary>
    public static async Task<int> AddOpenVPNServer(Config config, ProfileItem profileItem, bool toFile = true)
    {
        profileItem.ConfigType = EConfigType.OpenVPN;
        profileItem.CoreType = ECoreType.openvpn;
        profileItem.Address = profileItem.Address.TrimEx();
        profileItem.Port = profileItem.Port is > 0 and <= 65535 ? profileItem.Port : 1194;
        profileItem.Network = nameof(ETransport.raw);

        var extra = profileItem.GetProtocolExtra();
        if (extra.Flow is not "tcp" and not "udp")
        {
            profileItem.SetProtocolExtra(extra with { Flow = "udp" });
        }
    ...
    public static async Task<RetResult> AddGroupRegionServer(Config config, SubItem? subItem)
    {
        ...
        // Build a GeoIP country code cache for all child profiles (only IP addresses)
        var geoCache = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase);
        foreach (var p in childProfiles ?? [])
        {
            if (p is not { Address: { Length: > 0 } addr })
            {
                continue;
            }
            if (geoCache.ContainsKey(addr))
            {
                continue;
            }
        ...
        foreach (var regionFilter in PolicyGroupRegionFilters)
        {
            ...
            // Match child profiles by both regex on remarks AND GeoIP on address
            var matchedChildProfiles = childProfiles?.Where(p =>
                    p != null &&
                    p.IsValid() &&
                    !p.ConfigType.IsComplexType() &&
                    (IsRegionMatch(p, extraItem.Filter, regionFilter.Key, geoCache))
                )
                .ToList() ?? [];
    ...
    /// <summary>
    /// Check whether a profile matches a region, using both regex on remarks
    /// and GeoIP lookup on the address. Either match is sufficient.
    /// </summary>
    private static bool IsRegionMatch(ProfileItem p, string filter, string regionCode,
        Dictionary<string, string> geoCache)
    {
        // Regex match on remarks (existing behavior)
        if (filter.IsNotEmpty() && Regex.IsMatch(p.Remarks, filter))
        {
            return true;
        }

        // GeoIP match on address
        if (p.Address.IsNotEmpty()
            && geoCache.TryGetValue(p.Address, out var countryCode)
            && countryCode.Equals(regionCode, StringComparison.OrdinalIgnoreCase))
        {
            return true;
        }
    ...
    private static async Task<int> AddBatchServersCommon(Config config, string strData, string subid, bool isSub)
    {
        ...
        foreach (var str in arrData)
        {
            ...
            var addStatus = profileItem.ConfigType switch
            {
                ...
                EConfigType.OpenVPN => await AddOpenVPNServer(config, profileItem, false),
                _ => -1,
            };
    ...
    /// <summary>
    /// Import a complete OpenVPN client profile as a native-core custom entry.
    /// Keeping the original .ovpn text avoids losing provider-specific directives,
    /// inline certificates, routes, or auth plugins during URI-style normalization.
    /// </summary>
    private static async Task<int> AddBatchServers4OpenVPN(Config config, string strData, string subid, bool isSub)
    {
        if (!Regex.IsMatch(strData, @"(?im)^\s*client\s*$")
            || !Regex.IsMatch(strData, @"(?im)^\s*remote\s+\S+"))
        {
            return -1;
        }

        var tempPath = Path.Combine(Path.GetTempPath(), $"aogpn-openvpn-{Utils.GetGuid(false)}.ovpn");
        try
        {
            await File.WriteAllTextAsync(tempPath, strData);
            var item = new ProfileItem
            {
                ConfigType = EConfigType.Custom,
                CoreType = ECoreType.openvpn,
                Remarks = $"OpenVPN {DateTime.Now:yyyy-MM-dd HH:mm}",
                Address = tempPath,
                Subid = subid,
                IsSub = isSub,
            };

            return await AddCustomServer(config, item, blDelete: true) == 0 ? 1 : -1;
        }
        catch (Exception ex)
        {
            Logging.SaveLog($"{_tag} OpenVPN import", ex);
            return -1;
        }
        finally
        {
    ...
    private static async Task<int> AddBatchServers4Wireguard(Config config, string strData, string subid, bool isSub)
    {
        ...
        if (lstServer?.Count > 0)
        {
            ...
            foreach (var item in lstServer)
            {
                ...
                {
                    // Faz 3: içe aktarılan WireGuard peer'ını gpn_servers tablosuna yaz
                    // (istemci private key DPAPI ile şifrelenir). Upsert idempotent —
                    // ServerId uç noktadan türetildiği için yeniden içe aktarma yeni satır oluşturmaz.
    ...
    public static async Task<int> AddBatchServers(Config config, string strData, string subid, bool isSub)
    {
        ...
        //maybe complete OpenVPN client config
        if (counter < 1)
        {
            counter = await AddBatchServers4OpenVPN(config, strData, subid, isSub);
        }
    ...
    public static int SaveWindowSizeItem(Config config, string typeName, double width, double height, bool isMaximized = false, double? left = null, double? top = null)
    {
        var sizeItem = config?.UiItem?.WindowSizeItem?.FirstOrDefault(t => t.TypeName == typeName);
        if (sizeItem == null)
        {
        ...
        sizeItem.IsMaximized = isMaximized;

        // Position is optional so existing callers keep working unchanged; when a
        // window stores one, persist it (RestoreBounds when maximized, so the
        // restored window returns to the same spot).
        if (left is not null)
        {
