...
public class ProfileExManager
{
    ...
    /// <summary>Marks (or unmarks) a profile as a user favorite.</summary>
    public void SetFav(string indexId, bool fav)
    {
        var profileEx = GetProfileExItem(indexId);
        profileEx.IsFav = fav;
        IndexIdEnqueue(indexId);
    ...
    /// <summary>Returns the favorite flag for a profile (false when unknown).</summary>
    public bool GetFav(string indexId)
    {
        var profileEx = _lstProfileEx.FirstOrDefault(t => t.IndexId == indexId);
        return profileEx?.IsFav ?? false;
    }

    /// <summary>Records the current time as the profile's last-used stamp.</summary>
    public void TouchLastUsed(string indexId)
    {
        var profileEx = GetProfileExItem(indexId);
        profileEx.LastUsed = DateTimeOffset.UtcNow.ToUnixTimeMilliseconds();
        IndexIdEnqueue(indexId);
    }
}
