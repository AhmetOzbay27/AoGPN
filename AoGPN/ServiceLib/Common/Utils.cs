...
public class Utils
{
    ...
    /// <summary>
    /// Aktif fiziksel (uplink) arayüzü döner: Up durumunda, loopback/tun/vmware
    /// olmayan, link-local (169.254) olmayan IPv4'ü VE gerçek (non-APIPA) bir IPv4
    /// ağ geçidi olan NIC. sing-box TUN (auto_route + strict_route) aktifken sing-box'
    /// ın kendi WireGuard/endpoint ve direct soketleri bu arayüze bağlanırsa, paketler
    /// TUN'a geri girip sonsuz döngüye düşmez.
    /// </summary>
    /// <returns>(arayüz adı, birincil IPv4) — bulunamazsa (null, null).</returns>
    public static (string? Name, string? Ipv4) GetPhysicalDefaultInterface()
    {
        try
        {
            var candidates = NetworkInterface.GetAllNetworkInterfaces()
                ...
                .Select(n =>
                {
                    ...
                    var ipv4 = props.UnicastAddresses
                        ...
                        .Select(a => a.Address)
                        .FirstOrDefault(a => !IPAddress.IsLoopback(a) && !IsLinkLocalIPv4(a));
                    var hasGateway = props.GatewayAddresses.Any(g =>
                        g.Address.AddressFamily == System.Net.Sockets.AddressFamily.InterNetwork
                        && !IsLinkLocalIPv4(g.Address));
                    var name = n.Name ?? string.Empty;
                    var tunLike = name.Contains("tun", StringComparison.OrdinalIgnoreCase)
                        || name.Contains("vmnet", StringComparison.OrdinalIgnoreCase)
                        || name.Contains("vmware", StringComparison.OrdinalIgnoreCase)
                        || name.Contains("virtual", StringComparison.OrdinalIgnoreCase);
                    return new InterfaceProbe(name, n.NetworkInterfaceType, ipv4?.ToString(), hasGateway, tunLike);
                });
            return PickDefaultPhysicalInterface(candidates);
        }
        catch (Exception ex)
        {
            Logging.SaveLog("GetPhysicalDefaultInterface failed", ex);
        }
        return (null, null);
    }

    /// <summary>Arayüz seçim kararının girdisi — testlerde NIC listesi yokken üretilebilir.</summary>
    internal sealed record InterfaceProbe(
        string Name,
    ...
    /// <summary>
    /// Saf arayüz seçici (saf ve test edilebilir — ağ okumaz): ilk QUALIFYING fiziksel
    /// NIC'i döndürür. Koşullar:
    ///   • Tip Ethernet veya Wireless80211 olmalı (tünel sanal adaptörleri — wintun/
    ///     wireguard; bazı ortamlar onları Tunnel dışında bir tiple raporlayabilir —
    ///     name ile de elenir),
    ///   • Ad tun/vmnet/vmware/virtual içermemeli (IsTunLike),
    ///   • APIPA-olmayan bir IPv4'e sahip olmalı,
    ///   • Gerçek (non-APIPA) bir IPv4 ağ geçidi olmalı.
    /// Birden çok fiziksel NIC varsa <see cref="NetworkInterface.GetAllNetworkInterfaces"/>
    /// sırasındaki İLK uygun (varsayılan rota taşıyan) olan seçilir.
    /// </summary>
    internal static (string? Name, string? Ipv4) PickDefaultPhysicalInterface(IEnumerable<InterfaceProbe> candidates)
    {
        foreach (var ni in candidates)
        {
            if (ni.Type is not (NetworkInterfaceType.Ethernet or NetworkInterfaceType.Wireless80211))
            {
                continue;
            }
            if (ni.IsTunLike || string.IsNullOrEmpty(ni.IPv4) || !ni.HasRealGateway)
            {
                continue;
            }
            return (ni.Name, ni.IPv4);
        }
        return (null, null);
    }
    ...
    /// <summary>
    /// Waits until the OS reports an available network, then returns. Used at startup
    /// so auto-run (start on boot) does not start the proxy-only core before the
    /// network stack is up, which would leave the OS proxy pointing at a listener
    /// that cannot reach the node yet. Returns immediately when a network is already
    /// available, or after <paramref name="timeoutMs"/> to avoid blocking startup forever.
    /// </summary>
    public static async Task WaitForNetworkAsync(int timeoutMs = 30_000)
    {
        ...
        void OnChanged(object? sender, NetworkAvailabilityEventArgs e)
        {
            if (e.IsAvailable)
            {
    ...
    /// <summary>
    /// Yeni bir WireGuard istemci özel anahtarı üretir (base64). WireGuard özel
    /// anahtarı 32 rastgele bayttır ve tüm-sıfır olamaz; kriptografik güvenli RNG
    /// kullanılır. "Anahtar üret" butonu için (GpnServerEditViewModel).
    /// </summary>
    public static string GenerateWireGuardPrivateKey()
    {
        var bytes = new byte[32];
        RandomNumberGenerator.Fill(bytes);
        // Tüm-sıfır anahtar geçersizdir (WireGuard kuralları) — yeniden üret.
    ...
    public static string StartupPath()
    {
        ...
        {
            return Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "AoGPN");
        }
    ...
    {
        var tempPath = Path.Combine(StartupPath(), "Logs");
        if (!Directory.Exists(tempPath))
        {
            Directory.CreateDirectory(tempPath);
        }
    ...
    public static bool IsPackagedInstall()
    {
        try
        {
            ...
            if (p.StartsWith("/opt/AoGPN", StringComparison.OrdinalIgnoreCase))
            {
                return true;
            }

            if (p.StartsWith("/usr/lib/AoGPN", StringComparison.OrdinalIgnoreCase))
            {
                return true;
            }

            if (p.StartsWith("/usr/share/AoGPN", StringComparison.OrdinalIgnoreCase))
            {
                return true;
            }
        }
