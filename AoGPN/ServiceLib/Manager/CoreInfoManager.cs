...
public sealed class CoreInfoManager
{
    ...
    private void InitCoreInfo()
    {
        ...
        [
            new CoreInfo
                {
                    CoreType = ECoreType.AoGPN,
                    Url = GetCoreUrl(ECoreType.AoGPN),
                    ReleaseApiUrl = urlN.Replace(Global.GithubUrl, Global.GithubApiUrl),
                    DownloadUrlWin64 = urlN + "/download/{0}/AoGPN-windows-64.zip",
                    DownloadUrlWinArm64 = urlN + "/download/{0}/AoGPN-windows-arm64.zip",
                    DownloadUrlLinux64 = urlN + "/download/{0}/AoGPN-linux-64.zip",
                    DownloadUrlLinuxArm64 = urlN + "/download/{0}/AoGPN-linux-arm64.zip",
                    DownloadUrlLinuxRiscV64 = urlN + "/download/{0}/AoGPN-linux-riscv64.zip",
                    DownloadUrlLinuxLoong64 = urlN + "/download/{0}/AoGPN-linux-loong64.zip",
                    DownloadUrlOSX64 = urlN + "/download/{0}/AoGPN-macos-64.zip",
                    DownloadUrlOSXArm64 = urlN + "/download/{0}/AoGPN-macos-arm64.zip",
                },
                ...
                // OpenVPN is distributed independently from AoGPN because its
                // platform TAP/Wintun packages and service installation differ by OS.
                // The user can place the matching client in bin/openvpn (or select
                // an installed/copy-managed binary) and AoGPN supplies --config.
                new CoreInfo
                {
                    CoreType = ECoreType.openvpn,
                    CoreExes = ["openvpn"],
                    Arguments = "--config {0} --auth-nocache",
                    Url = GetCoreUrl(ECoreType.openvpn),
                    Match = "OpenVPN",
                    VersionArg = "--version",
                },
