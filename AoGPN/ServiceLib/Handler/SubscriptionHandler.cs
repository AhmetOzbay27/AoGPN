...
public static class SubscriptionHandler
{
    ...
    {
        Logging.Verbose("Subscription", "update_start", ("subId", subId), ("blProxy", blProxy));
        ...
        foreach (var item in subItem)
        {
            try
            {
                ...
                Logging.Verbose("Subscription", "fetching", ("remarks", item.Remarks), ("url", item.Url));
                ...
                Logging.Verbose("Subscription", "fetched",
                    ("remarks", item.Remarks), ("length", result.Length), ("success", result.IsNotEmpty()));
            ...
            catch (Exception ex)
            {
                ...
                Logging.Verbose("Subscription", "error", ("remarks", hashCode), ("error", ex.Message));
                await updateFunc?.Invoke(false, $"{hashCode}{ResUI.MsgFailedImportSubscription}: {ex.Message}");
                await updateFunc?.Invoke(false, "-------------------------------------------------------");
            }
        }

        Logging.Verbose("Subscription", "update_end", ("successCount", successCount), ("total", subItem.Count));
