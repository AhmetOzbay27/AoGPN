...
public class ProfilesViewModel : MyReactiveObject
{
    ...
    public ReactiveCommand<Unit, Unit> ScanDeadNodesCmd { get; }
    ...
    public ProfilesViewModel()
    {
        ...
        ScanDeadNodesCmd = ReactiveCommand.CreateFromTask(async () =>
        {
            await ScanDeadNodesAsync();
        });
        //servers export
    ...
    /// <summary>
    /// One-click scan all non-complex nodes with tcping, then remove dead ones.
    /// Uses the existing speedtest infrastructure and then runs RemoveInvalidServerResult.
    /// </summary>
    public async Task ScanDeadNodesAsync()
    {
        NoticeManager.Instance.Enqueue(ResUI.ScanDeadNodesStart);

        // Run tcping on all nodes (non-complex types)
        var allProfiles = JsonUtils.Deserialize<List<ProfileItem>>(
            JsonUtils.Serialize(ProfileItems?.OrderBy(t => t.Sort)));

        if (allProfiles is not { Count: > 0 })
        {
            NoticeManager.Instance.Enqueue(ResUI.ScanDeadNodesNone);
            return;
        }
        ...
        _speedtestService.RunLoop(ESpeedActionType.Tcping, allProfiles);

        // Poll for completion of speedtest, then clean
        _ = Task.Run(async () =>
        {
            // Wait for the speedtest to finish by polling
            await Task.Delay(2000); // initial delay for test to start
            var tries = 0;
            while (tries < 600) // max 10 minute wait
            {
                await Task.Delay(1000);
                tries++;

                // Check if all profiles have been tested (have non-zero delay)
                var profileExs = await ProfileExManager.Instance.GetProfileExs();
                var testedIds = new HashSet<string>(
                    profileExs.Where(pe => pe.Delay != 0).Select(pe => pe.IndexId));
                var allIds = new HashSet<string>(
                    allProfiles.Where(p => !p.ConfigType.IsComplexType()).Select(p => p.IndexId));

                if (allIds.Count > 0 && allIds.All(id => testedIds.Contains(id)))
                {
                    break;
                }

                // Also break if we got test results for at least 80% and it's been 30s
                var testedCount = allIds.Count(id => testedIds.Contains(id));
                if (tries > 30 && testedCount >= allIds.Count * 0.8)
                {
                    break;
                }
            }

            // Now clean dead nodes
            var removed = await ConfigHandler.RemoveInvalidServerResult(_config, _config.SubIndexId);

            RxSchedulers.MainThreadScheduler.Schedule(removed, (scheduler, removedCount) =>
            {
                _ = RefreshServers();
                if (removedCount > 0)
                {
                    NoticeManager.Instance.Enqueue(
                        string.Format(ResUI.ScanDeadNodesEnd, removedCount));
                }
                else
                {
                    NoticeManager.Instance.Enqueue(ResUI.ScanDeadNodesNone);
