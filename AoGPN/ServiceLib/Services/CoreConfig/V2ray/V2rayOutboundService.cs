                ...
                case EConfigType.WireGuard:
                    {
                        ...
                        var peer = new WireguardPeer4Ray
                        {
                            ...
                            keepAlive = protocolExtra.WgPersistentKeepalive is > 0
    ...
    private void FillBoundStreamSettings(Outbounds4Ray outbound)
    {
        try
        {
            ...
            if (_node.StreamSecurity == Global.StreamSecurity)
            {
                ...
                // NOTE: Xray 26 removed the allowInsecure option entirely
                // (migrated to pinnedPeerCertSha256 / verifyPeerCertByName).
                // Writing it — even as false — fails config validation, so it is
                // intentionally left null here and omitted from the JSON.
