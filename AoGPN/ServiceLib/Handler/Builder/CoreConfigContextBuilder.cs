...
public class CoreConfigContextBuilder
{
    ...
    public static async Task<CoreConfigContextBuilderResult> Build(Config config, ProfileItem node, bool proxyOnly)
    {
        ...
        // GPN/TUN capture is sing-box only. The GPN process_name routing rules,
        // sniff-based DoH hijacking and the QUIC block are generated by the
        // sing-box config builder, and Xray has no native TUN support on
        // Windows. sing-box supports every in-app protocol (VLESS, VMess,
        // Trojan, Shadowsocks, Hysteria2, TUIC, WireGuard, Anytls, Naive, ...),
        // so when the TUN is enabled the main core must be sing-box — never
        // Xray/v2fly. Custom configs and OpenVPN keep their own core.
        ...
        var context = new CoreConfigContext
        {
            ...
            ProtectCoreTypeList = isTunEnabled ? [ECoreType.Xray, ECoreType.sing_box] : []
        };
        ...
        // must not be interpreted as an Xray/sing-box outbound or inherit their
    ...
    {
        // OpenVPN provides the tunnel itself; attaching the sing-box legacy
        // protection helper would create a second competing TUN lifecycle.
        ...
        // interface-name pinning — a sing-box legacy protect helper would fight
        ...
        // enabled the main core is forced to sing-box, and GetPreSocksItem only
        // builds a helper for non-sing-box cores. Recomputing via GetCoreType
        // here would see the profile's raw Xray type and wrongly spawn a
        // sing-box pre-socks helper on top of the sing-box main core.
