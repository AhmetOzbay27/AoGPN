...
MIN_KERNEL="6.12"
PKGROOT="AoGPN-publish"
PROJECT_HINT="AoGPN.Desktop/AoGPN.Desktop.csproj"
...
prepare_workspace() {
  ...
  [[ -f "$PROJECT" ]] || PROJECT="$(find . -maxdepth 3 -name 'AoGPN.Desktop.csproj' | head -n1 || true)"
  [[ -f "$PROJECT" ]] || die "AoGPN.Desktop.csproj not found"
}
...
choose_channel() {
  ...
  if [[ -t 0 ]]; then
    echo "[?] Choose AoGPN release channel:" >&2
...
get_latest_tag_latest() {
  curl -fsSL "https://api.github.com/repos/AhmetOzbay27/AoGPN/releases/latest" \
    | jq -re '.tag_name' \
    | sed 's/^v//'
}

get_latest_tag_prerelease() {
  curl -fsSL "https://api.github.com/repos/AhmetOzbay27/AoGPN/releases?per_page=20" \
...
bundle_url_for_rid() {
  ...
  case "$rid" in
    linux-x64)   echo "https://raw.githubusercontent.com/AhmetOzbay27/AoGPN-core-bin/refs/heads/master/AoGPN-linux-64.zip" ;;
    linux-arm64) echo "https://raw.githubusercontent.com/AhmetOzbay27/AoGPN-core-bin/refs/heads/master/AoGPN-linux-arm64.zip" ;;
    *)           return 1 ;;
  esac
}
...
populate_assets_zip_mode() {
  ...
  echo "[+] Try AoGPN bundle archive: $url"

  tmp="$(mktemp -d)"
  curl -fL "$url" -o "$tmp/aogpn.zip" || { echo "[!] Bundle download failed"; rm -rf "$tmp"; return 1; }
  unzip -q "$tmp/aogpn.zip" -d "$tmp" || { echo "[!] Bundle unzip failed"; rm -rf "$tmp"; return 1; }
  ...
  rm -f "$outroot/aogpn.zip" 2>/dev/null || true
  find "$outroot" -type d -name "mihomo" -prune -exec rm -rf {} + 2>/dev/null || true

  nested_dir="$(find "$outroot" -maxdepth 1 -type d -name 'AoGPN-linux-*' | head -n1 || true)"
...
rid_host_arch() {
  case "$1" in
    ...
    linux-loongarch64) echo "loongarch64" ;;
    linux-riscv64)     echo "riscv64" ;;
    *)                 return 1 ;;
  esac
}

# Uses the app's own headless updater (AoGPN.CoreTool --update-cores) to fetch the
# host-architecture sing-box/Xray cores plus geo assets, exactly like the app does
# at build time on Windows. The tool runs on any platform (plain net10.0, no WPF),
# so packaging hosts produce the same bin layout as the MSBuild download target.
populate_assets_tool_mode() {
  local outroot="$1"
  local tool_src=""
  local tool_dir=""
  local tool_exe=""

  tool_src="$(find . -maxdepth 4 -name 'AoGPN.CoreTool.csproj' | head -n1 || true)"
  [[ -n "$tool_src" ]] || { echo "[CoreTool] AoGPN.CoreTool.csproj not found"; return 1; }

  tool_dir="$(mktemp -d)"

  echo "[*] Publishing AoGPN.CoreTool..."
  if ! dotnet publish "$tool_src" -c Release -o "$tool_dir" -p:PublishSingleFile=false >/dev/null 2>&1; then
    echo "[CoreTool] publish failed"
    rm -rf "$tool_dir"
    return 1
  fi

  tool_exe="$tool_dir/AoGPN.CoreTool"
  [[ -f "$tool_exe" ]] || { echo "[CoreTool] binary not found after publish"; rm -rf "$tool_dir"; return 1; }

  echo "[*] Running AoGPN.CoreTool --update-cores (fetches host-arch cores + geo)..."
  if ! (cd "$tool_dir" && ./AoGPN.CoreTool --update-cores); then
    echo "[CoreTool] --update-cores failed"
    rm -rf "$tool_dir"
    return 1
  fi
  ...
  if [[ -d "$tool_dir/bin" ]]; then
    mkdir -p "$outroot/bin"
    rsync -a "$tool_dir/bin/" "$outroot/bin/"
    unify_geo_layout "$outroot"
    rm -rf "$tool_dir"
    echo "[*] CoreTool assets staged."
    return 0
  fi

  echo "[CoreTool] no bin output produced"
  rm -rf "$tool_dir"
  return 1
}
...
stage_runtime_assets() {
  ...
  # The headless CoreTool downloads the HOST architecture's cores, so it is only
  # usable when packaging for the host's own architecture.
  target_arch="$(rid_host_arch "$rid" || true)"

  if [[ -n "$target_arch" && "$target_arch" == "$(uname -m)" ]] && command -v dotnet >/dev/null 2>&1; then
    if populate_assets_tool_mode "$outroot"; then
      echo "[*] Using AoGPN.CoreTool --update-cores assets."
      return 0
    fi
    echo "[*] CoreTool path failed, falling back to bundle assets."
  fi

  if [[ "$FORCE_NETCORE" -eq 0 ]]; then
    if populate_assets_zip_mode "$outroot" "$rid"; then
      echo "[*] Using AoGPN bundle archive."
    else
...
Version:        __VERSION__
Release:        1%{?dist}
Summary:        AoGPN (Avalonia) GUI client for Linux
License:        GPL-3.0-only
URL:            https://github.com/AhmetOzbay27/AoGPN
BugURL:         https://github.com/AhmetOzbay27/AoGPN/issues
...
%description
AoGPN Linux for Red Hat Enterprise Linux
Support vless / vmess / Trojan / http / socks / Anytls / Hysteria2 / Shadowsocks / tuic / WireGuard
Support Red Hat Enterprise Linux / Fedora Linux / Rocky Linux / AlmaLinux / CentOS
For more information, Please visit our website
https://github.com/AhmetOzbay27/AoGPN
...
%install
install -dm0755 %{buildroot}/opt/AoGPN
cp -a * %{buildroot}/opt/AoGPN/

find %{buildroot}/opt/AoGPN -type d -exec chmod 0755 {} +
find %{buildroot}/opt/AoGPN -type f -exec chmod 0644 {} +
[ -f %{buildroot}/opt/AoGPN/AoGPN ] && chmod 0755 %{buildroot}/opt/AoGPN/AoGPN || :

install -dm0755 %{buildroot}%{_bindir}
install -m0755 /dev/stdin %{buildroot}%{_bindir}/aogpn << 'EOF'
#!/usr/bin/bash
set -euo pipefail
DIR="/opt/AoGPN"

if [[ -x "$DIR/AoGPN" ]]; then exec "$DIR/AoGPN" "$@"; fi

for dll in AoGPN.Desktop.dll AoGPN.dll; do
  if [[ -f "$DIR/$dll" ]]; then exec /usr/bin/dotnet "$DIR/$dll" "$@"; fi
done

echo "AoGPN launcher: no executable found in $DIR" >&2
ls -l "$DIR" >&2 || true
exit 1
EOF

install -dm0755 %{buildroot}%{_datadir}/applications
install -m0644 /dev/stdin %{buildroot}%{_datadir}/applications/aogpn.desktop << 'EOF'
[Desktop Entry]
Type=Application
Name=AoGPN
Comment=AoGPN for Red Hat Enterprise Linux
Exec=aogpn
Icon=aogpn
Terminal=false
Categories=Network;
...
install -m0644 %{_builddir}/__PKGROOT__/aogpn.png %{buildroot}%{_datadir}/icons/hicolor/256x256/apps/aogpn.png
...
%{_bindir}/aogpn
/opt/AoGPN
%{_datadir}/applications/aogpn.desktop
%{_datadir}/icons/hicolor/256x256/apps/aogpn.png
SPEC
...
package_binary() {
  ...
  icon_candidate="$project_dir/AoGPN.png"
  [[ -f "$icon_candidate" ]] || { echo "Required icon not found: $icon_candidate"; return 1; }
  cp "$icon_candidate" "$workdir/$PKGROOT/aogpn.png"
  ...
  for f in "${RPM_TOPDIR}/RPMS/${archdir}/AoGPN-${VERSION}-1"*.rpm; do
    [[ -e "$f" ]] || continue
    echo "  $f"
    BUILT_RPMS+=("$f")
  done
